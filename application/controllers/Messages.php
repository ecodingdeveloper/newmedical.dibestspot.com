...
class Messages extends CI_Controller
{
  ...
  public function index()
  {
    ...
    if ($this->session->userdata('role') == '1' || $this->session->userdata('role') == '6') {
      ...
      $this->data['users'] =  $this->messages->get_patients($user_id);
      $this->load->vars($this->data);
      $this->load->view($this->data['theme'] . '/template');
    } else {
      $this->data['module']    = 'messages';
      $this->data['page'] = 'index';
      $this->data['users'] =  $this->messages->get_doctors($user_id);
      $this->load->vars($this->data);
      $this->load->view($this->data['theme'] . '/template');
    }
  }
  ...
  public function search_users()
  {
    ...
    if ($this->session->userdata('role') == '1' || $this->session->userdata('role') == '6') {
      $users =  $this->messages->get_patients($user_id, $keywords);
    } else {
      $users =  $this->messages->get_doctors($user_id, $keywords);
    }
    ...
    $response = array();
    $result = array();
    $adminChat = $this->messages->get_admin_chats($user_id);

    if (!empty($users)  || !empty($adminChat)) {
      foreach ($users as $rows) {
        ...
        if (!empty($adminChat)) {
          ...
          $data['admin_role'] = 1;
          $data['profileimage'] = (!empty($adminChat[0]['profileimage'])) ? base_url() . $adminChat[0]['profileimage'] : base_url() . 'assets/img/user.png';
          $data['userid'] = $adminChat[0]['userid'];
          $data['username'] = $adminChat[0]['username'];
          $data['first_name'] = ucfirst($adminChat[0]['first_name']);
          $data['last_name'] = ucfirst($adminChat[0]['last_name']);
          $data['role'] = $adminChat[0]['role'];
          $data['type'] = $adminChat[0]['type'];
          $data['unread_count'] = $adminChat[0]['unread_count'];
          $data['last_msg'] = strval($adminChat[0]['last_msg']);
          if ($adminChat[0]['chatdate'] != null) {
            $data['chatdate'] = time_elapsed_string($adminChat[0]['chatdate']);
          } else {
            $data['chatdate'] = "";
          }
          $data['online_status'] = "avatar-offline";
  ...
  public function get_chat_user()
  {
    ...
    $data['status'] = 500;
    if ($this->session->userdata('role') == '1' || $this->session->userdata('role') == '6') {
      $users =  $this->messages->get_patients($user_id);
      if (!empty($users)) {
      ...
      $users =  $this->messages->get_doctors($user_id);
      if (!empty($users)) {
        $data['user_id'] = $users[0]['userid'];
        $data['username'] = $users[0]['username'];
        $data['name'] = $users[0]['first_name'] . ' ' . $users[0]['last_name'];
        $data['admin_role'] = 1; //New
        $data['status'] = 200;
      }
      $adminChat = $this->messages->get_admin_chats($user_id);
      if (!empty($adminChat)) {
  ...
  public function get_chat_img()
  {
    ...
    if($admin_role==1){
      $result = $this->db->select('profileimage')->where('id', $user_id)->get('administrators')->row_array();
      $status_result=array();
    } else {
  ...
  public function get_messages()
  {
    ...
    $latest_chat = $this->messages->get_latest_chat($selected_user, $user_id,$admin_role);
    $total_chat = $this->messages->get_total_chat_count($selected_user, $user_id,$admin_role);
    $last_message_id = $this->messages->get_last_message_id($selected_user, $user_id,$admin_role);
  ...
  public function insert_chat()
  {
    ...
    $latest_chat = $this->api->get_latest_chat($this->get_recieved_id($data['is_admin']), $this->session->userdata('user_id'),$data['is_admin']);
  ...
  public function get_recieved_id($admin_role)
  {
    if($admin_role=='0') {
      ...
      $where = array('name' => $_POST['to_username']);
      return $this->db->get_where('administrators', $where)->row()->id;
    }
  }
  ...
  public function upload_files()
  {
    ...
    try {
      ...
      if ($file_type != "jpg" && $file_type != "png" && $file_type != "jpeg" && $file_type != "gif") {
        $errMsg = $this->language['lg_invalid_extensi'].". ".$this->language['lg_allowed_jpg_gif1'];
      ...
      if ($this->upload->do_upload('userfile')) {
        ...
        $data = array(
          ...
          'sent_id' => $this->session->userdata('user_id'),
          'msg' => 'file',
          'file_name' => $file_name,
          ...
          'time_zone' => $this->session->userdata('time_zone'),
          'file_path' => $path,
          'is_admin'=> $this->input->post('admin_role')
        );
  ...
  public function get_old_messages()
  {
    ...
    $user_id = $this->session->userdata('user_id');
    $selected_user = $_POST['selected_user_id'];
    $latest_chat = $this->messages->get_old_chat($selected_user, $user_id, $total);
  ...
  public function get_message_counts()
  {
    ...
    $data = $this->db
      ->select('a.username, COUNT(c.id) as count')
      ->join('users a', 'a.id = c.sent_id', 'left')
      ->group_by('c.sent_id, a.username') // Include all non-aggregated columns
      ->get_where('chat c', $where)->result();
