...
class Chat_model extends CI_Model
{
    ...
    u.username,
    u.profileimage,
    (select chatdate from chat where recieved_id = u.id OR sent_id = u.id ORDER BY chatdate DESC LIMIT 1) as chatdate,
    (select msg from chat where sent_id = u.id OR recieved_id = u.id ORDER BY id DESC LIMIT 1) as last_msg,
    (select type from chat where sent_id = u.id OR recieved_id = u.id ORDER BY id DESC LIMIT 1) as type,
    (select date_time from user_online_status where user_id = u.id) as date_time,
    (select time_zone from user_online_status where user_id = u.id) as time_zone,
    (select count(id) from chat where recieved_id = 1 and sent_id = u.id and read_status = 0) as unread_count
");
$this->db->from('users u');
$this->db->join('chat c', 'u.id = c.recieved_id', 'left');
$this->db->where('c.is_admin', '1');
$this->db->where('c.recieved_id >', '1');
if (!empty($keywords)) {
    $this->db->group_start();
...
$this->db->group_by('u.id, u.first_name, u.last_name, u.role, u.username, u.profileimage');
...
$result = $this->db->get()->result_array();
    ...
    public function get_latest_chat($selected_user, $user_id,$admin_role=0)
    {
        ...
        where  msg.is_admin = 1 AND ((msg.recieved_id = $selected_user AND msg.sent_id = $user_id) or  (msg.recieved_id = $user_id AND msg.sent_id =  $selected_user))   ORDER BY msg.id ASC LIMIT $total,$per_page ");
    ...
    public function get_old_chat($selected_user, $user_id, $total)
    {
        ...
        /*$totalChat =  $this->get_total_chat_count($selected_user, $user_id,1);
        if ($totalChat > 5) {
            $totalChat = $totalChat - 5;
        } else {
            $totalChat = 0;
        }

        $offset = max(0, $totalChat - 5);*/
    ...
    public function get_total_chat_count($selected_user, $user_id,$admin_role)
    {
        ...
        $sql = "SELECT msg.id  from chat msg
    left join chat_deleted_details cd on cd.chat_id  = msg.id
    where   msg.is_admin = $admin_role AND ((msg.recieved_id = $selected_user AND msg.sent_id = $user_id) or  (msg.recieved_id = $user_id AND msg.sent_id =  $selected_user)) GROUP BY msg.id  ORDER BY msg.id DESC ";
        //echo  $sql;
